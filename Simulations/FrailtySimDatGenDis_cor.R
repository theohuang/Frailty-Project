...
## Using correlated distribution
## Last udpated: February 21, 2020

source("Frailty Simulation Functions.R")


library(BayesMendel5)
library(dplyr)
library(abind)
library(doParallel)
...
f.w <- rep(0, nrow(supp.w))
for(i in 1:length(seq(0, 3, 0.5))){
  f.w[abs(supp.w$W.BC - supp.w$W.OC) == seq(0, 3, 0.5)[i]] <-
    length(seq(0, 3, 0.5)) - i + 1
}
f.w <- f.w / sum(f.w)
...
# mean.w.b <- sum(w.list.b * f.w.b)
# mean.w.o <- sum(w.list.o * f.w.o)
# var.w.b <- sum(w.list.b^2 * f.w.b) - mean.w.b^2
# var.w.o <- sum(w.list.o^2 * f.w.o) - mean.w.o^2
# 
# cor.w <- (sum(supp.w$W.BC * supp.w$W.OC * f.w) - mean.w.b * mean.w.o) /
#   sqrt(var.w.b * var.w.o)
...
save(fam.sim, file = paste(getwd(), "/Frailty/Simulations/simdat_dis_cor.RData", sep = ""))
save(fam.sim.bc, file = paste(getwd(), "/Frailty/Simulations/simdat_bc_dis_cor.RData", sep = ""))
